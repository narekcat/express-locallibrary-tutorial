...
exports.book_delete_post = function(req, res, next) {
    ...
    var bookId = mongoose.Types.ObjectId(req.body.bookid);
    async.parallel({
        book: function(callback) {
            Book.findById(bookId).exec(callback);
        },
        ...
        if (err) { return next(err); }
        if (results.bookisntances.length > 0) {
            res.render('book_delete', { title: 'Book Delete', book: results.book, bookinstances: results.bookinstnaces });
            return;
        } else {
            Book.findAndRemoveById(bookId, function deleteBook(err) {
                if (err) { return next(err); }
                res.redirect('/catalog/books');
            });
        }
    ...
    (req, res, next) => {
        ...
        if (!errors.isEmpty()) {
            ...
            async.parallel({
                ...
                res.render('book_from', { title: 'Update Book', authors: results.authors, genres: results.genres, book: book, errors: errors.array() });
            });
            return;
        } else {
